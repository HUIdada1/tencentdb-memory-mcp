...
(function () {
  ...
  /* ---------- 顶栏 tab ---------- */
  // 本期生效的页面：home / memory / agent / settings
  ...
  function renderStats(m, snap) {
    const online = snap.panelOk !== false && m.latency > 0;
    const st = $('#s-status');
    if (st) st.textContent = snap.panelOk === false ? '离线' : (online ? '在线' : '待连接');
    setCls($('#c-status'), snap.panelOk === false ? 'err' : (online ? 'ok' : ''));
  ...
  async function tick(snap) {
    ...
    const up = $('#h-uptime'); if (up) up.textContent = fmtDur(snap.uptime);
    const hp = $('#h-panel');
  ...
  function onHomeShown() {
    tdai.metricsGet().then(tick).catch(() => { });
    refreshDaemon(false);
