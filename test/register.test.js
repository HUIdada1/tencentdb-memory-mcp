...
const CLIENTS = ['ZCode CLI', 'Claude Code', 'Cursor', 'Codex'];
const KEYS = ['zcode', 'claude-code', 'cursor', 'codex'];
const find = (arr, key) => arr.find((x) => x.key === key);
const read = (p) => fs.readFileSync(path.join(HOME, p), 'utf8');
const readJson = (p) => JSON.parse(read(p));

// 造 4 个「已安装」客户端（各自带原有配置，验证不被破坏）
...
check('初始：4 个已装客户端均「未接入」', KEYS.every((k) => find(s0, k).status === 'missing'));
check('初始：指令文件未写入', find(s0, 'instructions').status === 'missing');
const sEmpty = register.status({ home: HOME_EMPTY, exePath: EXE });
check('空 HOME：客户端全部「未装客户端」', ['zcode', 'claude-code', 'cursor', 'codex'].every((k) => find(sEmpty, k).status === 'absent'));
...
check('注册：4 个客户端均写入', CLIENTS.every((n) => r1.some((x) => x.target === n && ['新增', '追加', '覆盖'].includes(x.action))));
check('注册：无失败项', !r1.some((x) => x.action === '失败'));
check('注册：未安装客户端不造配置', !fs.existsSync(path.join(HOME, '.trae')) && !fs.existsSync(path.join(HOME, '.gemini')));
...
check('接入后：4 个客户端均「已接入」', ['zcode', 'claude-code', 'cursor', 'codex'].every((k) => find(s1, k).status === 'installed'));
...
check('再跑一次：hook 跳过（脚本仍更新）', r2.find((x) => x.target === 'Claude Code hook').action === '跳过');
